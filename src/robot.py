...
class Robot(object):
    ...
    def fkine_fright_base(self):
        ...
        """
        d = 7
        q1 = -self.q[d+0]; q2 = self.q[d+1]+self.ang; q3 = self.q[d+2] ;
    ...
    def fkine_fleft_base(self):
        ...
        """
        d = 7+3
        q1 = self.q[d+0]; q2 = self.q[d+1]+self.ang2; q3 = self.q[d+2]
        T01 = np.dot(Ttransl([self.ltx, self.lty+self.bias2, -self.ltz]),
                     Trotx(q1))
        T12 = Troty(q2)
        T23 = np.dot(Ttransl([0., 0., -self.l1+self.bias3]),
                     Troty(q3))
        T34 = Ttransl([0., 0., -self.l2 +self.bias4])
        T = np.dot(np.dot(np.dot(T01, T12), T23), T34)
        return T
    ...
    def fkine_rright_base(self):
        ...
        d = 7+6
        q1 = -self.q[d+0]; q2 = self.q[d+1]+self.ang; q3 = self.q[d+2]
        T01 = np.dot(Ttransl([-(self.ltx)+self.bias, -self.lty, -self.ltz]),
                     Trotx(q1))
        T12 = Troty(q2)
        T23 = np.dot(Ttransl([0., 0., -self.l1]),
                     Troty(q3))
    ...
    def fkine_rleft_base(self):
        ...
        d = 7+9
        q1 = self.q[d+0]; q2 = self.q[d+1]+self.ang2; q3 = self.q[d+2]
        T01 = np.dot(Ttransl([-(self.ltx)+self.bias, self.lty+self.bias2, -self.ltz]),
                     Trotx(q1))
        T12 = Troty(q2)
        T23 = np.dot(Ttransl([0., 0., -self.l1+self.bias3]),
                     Troty(q3))
